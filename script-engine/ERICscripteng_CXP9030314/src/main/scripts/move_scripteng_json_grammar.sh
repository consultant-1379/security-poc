...
# the cm-editor name is due to avoid issue during the upgrade.  with the same name the existing files will be removed and updated.  
# if we change the "cm-editor" name none remove the existing files and link.
APP_NAME="cm-editor"
VM_DIRECTORY="/ericsson/script-engine/data/json"
...
create_symbolic_links(){
    $_LN -s ${APP_DST_DIR}/*.json ${JSON_DIRECTORY}
        if [ $? -ne 0 ];then
