...
public final class ValidationUtils {
    ...
    private static final String DURATION_PATTERN = "^(?!P0*Y?0*M?0*D?T?0*H?0*M?([0-9]*S)?$)P([0-9]*Y)?([0-9]*M)?([0-9]*D)?T?([0-9]*H)?([0-9]*M)?([0-9]*S)?$";
    ...
    /**
     * Method for checking if collection is null or empty
     *
     * @param givenCollection
     *            collection of objects
     * @return true or false
     */
    public static boolean isNullOrEmpty(final Collection<?> givenCollection) {
        if (givenCollection == null || givenCollection.isEmpty()) {
            return true;
        }
        return false;
    }
