...
public class EntitiesResource {
    ...
    public Response reissueEntityCertificate(final String entityReissueInfo) throws JsonProcessingException, IOException {
        ...
        try {
        response = Response.ok(commonUtil.getStreamingOutput(file), MediaType.APPLICATION_OCTET_STREAM).header("Content-Disposition", "attachment; filename=\"" + file.getName() + "\"")
                    .build();
        }
        finally {
            if (file.exists()) {
