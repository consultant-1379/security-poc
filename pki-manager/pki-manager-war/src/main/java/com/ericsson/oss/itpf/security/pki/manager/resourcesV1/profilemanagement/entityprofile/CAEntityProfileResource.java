...
/**
 * Rest service for fetching {@link EntityProfile} related to CA entity.
 * 
 * @author tcspred
 * @version 1.1.30
 * 
 */
@Path("/1.0/caentityprofile")
public class CAEntityProfileResource {
    ...
    /**
     * This method fetches id and names of all entity profiles related to ca entity.
     * 
    ...
    public Response fetch() throws JsonProcessingException {
        ...
        logger.debug("Fetching entity profiles related to CA entity.");

        final Profiles profiles = pkiManagerEServiceProxy.getProfileManagementService().exportProfiles(ProfileType.ENTITY_PROFILE);

        final List<EntityProfile> entityProfiles = getEntityProfilesForCAEntity(profiles);
        ...
        logger.debug("Successfully fetched entity profiles related to CA entity. {}", result);

        return Response.status(Status.OK).entity(result).build();

    }

    private List<EntityProfile> getEntityProfilesForCAEntity(final Profiles profiles) {
        final List<EntityProfile> caEntityProfiles = new ArrayList<EntityProfile>();

        for (final EntityProfile entityProfile : profiles.getEntityProfiles()) {

            if (entityProfile.getCertificateProfile().isForCAEntity()) {
                caEntityProfiles.add(entityProfile);
            }
        }

        return caEntityProfiles;
    }
}
