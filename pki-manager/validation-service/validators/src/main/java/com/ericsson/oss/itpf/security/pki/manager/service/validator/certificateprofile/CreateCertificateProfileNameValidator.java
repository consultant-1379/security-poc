...
/**
 * This class validates the Certificate Profile Name while creating the certificate for a {@link CertificateProfile}
 */
public class CreateCertificateProfileNameValidator extends AbstractProfileNameValidator implements CommonValidator<CertificateProfile> {
    ...
    private void validateCertificateProfileName(final CertificateProfile certificateProfile) throws InvalidProfileAttributeException, ProfileAlreadyExistsException, ProfileServiceException {

        logger.debug("Validating  Name in {} {} ", certificateProfile.getType(), certificateProfile.getName());

        certificateProfile.setName(certificateProfile.getName().trim());
        checkProfileNameFormat(certificateProfile.getName());
        checkProfileNameAvailability(certificateProfile.getName(), CertificateProfileData.class);
