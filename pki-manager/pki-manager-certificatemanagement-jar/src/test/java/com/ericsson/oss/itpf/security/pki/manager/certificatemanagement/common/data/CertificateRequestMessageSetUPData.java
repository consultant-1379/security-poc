...
public class CertificateRequestMessageSetUPData {
     ...
     * @throws OperatorCreationException
     */
    public CertificateRequestMessage generateEcCRMFRequest(final X500Name subject, final String sanExtension) throws IOException, NoSuchAlgorithmException, OperatorCreationException {
        ...
        final ProofOfPossessionSigningKeyBuilder poposkBuilder = new ProofOfPossessionSigningKeyBuilder(certRequest);
        final POPOSigningKey poposk = poposkBuilder.build(new JcaContentSignerBuilder(EC_SIGNATURE_ALGORITHM).setProvider(PROVIDER).build(keyPair.getPrivate()));
        final ProofOfPossession popo = new ProofOfPossession(poposk);
