...
public class PKCS10CertificationRequestSetUPData {
    ...
    private static final String EC_SIGNATURE_ALGORITHM = "SHA1withECDSA";

    public static final String KEY_GEN_ALGORITHM = "RSA";

    public static final String EC_KEY_GEN_ALGORITHM = "ECDSA";
     ...
     * @throws InvalidKeyException
     */
    public PKCS10CertificationRequest generateEcPKCS10Request(final X500Name subject, final String... sanAttributes) throws InvalidKeyException, IOException, NoSuchAlgorithmException, SignatureException {

        final KeyPair keyPair = setUPData.generateKeyPair(EC_KEY_GEN_ALGORITHM, 256);
        final AlgorithmIdentifier sigAlgId = new DefaultSignatureAlgorithmIdentifierFinder().find(EC_SIGNATURE_ALGORITHM);
        final SubjectPublicKeyInfo subjectPublicKeyInfo = SubjectPublicKeyInfo.getInstance(keyPair.getPublic().getEncoded());

        final DERSet attributes = getDERSet(sanAttributes);

        final CertificationRequestInfo certificationRequestInfo = new CertificationRequestInfo(subject, subjectPublicKeyInfo, attributes);
        final Signature signature = Signature.getInstance(EC_SIGNATURE_ALGORITHM);
        signature.initSign(keyPair.getPrivate());
        signature.update(certificationRequestInfo.getEncoded(ASN1Encoding.DER));
