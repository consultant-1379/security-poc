...
#*****************************************************************************#
#This function drops the pkiratdpsdb database,if database exists in pg_database
#*****************************************************************************#
function drop_db(){
...
EOF
    ...
    is_db_pkiratdpsdb=`grep -i ${DB} ${DB_TEST}.${LDATE} | wc -l`

    if [ ${is_db_pkiratdpsdb} -gt 0 ]; then
