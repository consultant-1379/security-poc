...
###########################################################################
# This script should be executed as a pki-ra-tdps post-install script in
# order to create schema for the pkiratdpsdb.
#
# Pre-requisite : Database pkiratdpsdb is already created
###########################################################################
...
source /ericsson/pki_postgres/db/pkiratdps/lib/postgres01.config
...
function installUpgradeDB() {
    case "${CURRENT_VERSION}" in
        "0.0.0") echo "Installing default version 1.0.0"
                ...
                checkExitCode "Creating all required tables with default values in pkiratdpsdb database for version 1.0.0"
                ;&
        "1.0.0") echo "Upgrading to version 2.0.0"
                ...
                checkExitCode "Revoking super user permissions for pkiratdps role, creating pkiratdpsgrp group and revoking public connections on pkiratdpsdb database in version 2.0.0"
                ;&
    esac
}
