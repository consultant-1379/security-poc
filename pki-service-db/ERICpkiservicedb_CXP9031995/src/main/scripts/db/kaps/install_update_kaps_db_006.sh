...
# the kapsdb.
#
# Pre-requisite : Database kapsdb is already created
###########################################################################
...
source /ericsson/pki_postgres/db/kaps/scripts/create_kaps_internaluser.sh
...
#*****************************************************************************
# In this method, kaps database will be installed or upgraded with roles,
...
function installUpgradeDB() {
    case "${CURRENT_VERSION}" in
        ...
        "0.0.0") echo "Installing default version 1.0.0"
                ...
                checkExitCode "Created all required tables with default values in kapsdb database for version 1.1.0"
                ;&

        "1.1.0") echo "Installing default version 1.1.0"
                createkapsInternalRole
                revokeOldUserAccess
                revokeSuperUserPermissions $DB_ROLE
                createGroupAndAddRoleToGroup $DB $DB_GROUP $DB_ROLE
                createGroupAndAddRoleToGroup $DB $DB_GROUP $DB_INTERNAL_ROLE
                alterSymmetricKeyTableOwner
                applySchema $DB $DDLS_PATH/insert.dbversion_1.2.0.ddl
                checkExitCode "Created kapsinternalrole and kapsgrp. Altered schema owner to kapsgrp. Created required tables with default values in kapsdb for version 1.2.0"
                ;&

        "1.2.0") echo "Upgrading to version 2.0.0"
                ...
                checkExitCode "Revoked public connections on kapsdb database in version 2.0.0"
                ;&
        "2.0.0") echo "Upgrading to version 2.1.0"
                applySchemaUsingRole $DB $DB_GROUP $DDLS_PATH/insert.dbversion_2.1.0.ddl
                checkExitCode "Role based password authentication on kapsdb database in version 2.1.0"
                ;&
    esac
}
