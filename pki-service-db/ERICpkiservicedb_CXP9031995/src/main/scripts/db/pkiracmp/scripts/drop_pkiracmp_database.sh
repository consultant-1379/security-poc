...
#*****************************************************************************#
#This function drops the pkiracmpdb database,if database exists in pg_database
#*****************************************************************************#
function drop_db(){
...
EOF
    ...
    is_db_pkiracmpdb=`grep -i ${DB} ${DB_TEST}.${LDATE} | wc -l`

    if [ ${is_db_pkiracmpdb} -gt 0 ]; then
