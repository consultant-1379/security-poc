...
source /ericsson/pki_postgres/db/pkirascep/lib/db-upgrade-shared-library.sh
...
#*****************************************************************************#
# In this method, pki-ra-scep database will be installed or upgraded with roles,
...
function installUpgradeDB() {
    case "${CURRENT_VERSION}" in
        "0.0.0") echo "Installing default version 1.0.0"
                ...
                checkExitCode "Creating all required tables with default values in pkirascepdb database for version 1.0.0"
                ;&
        "1.0.0") echo "Upgrading to version 2.0.0"
                ...
                checkExitCode "Revoking super user permissions for pkirascep role, creating pkirascepgrp group and revoking public connections on pkirascepdb database in version 2.0.0"
                ;&
        "2.0.0") echo "Upgraded to version 2.1.0"
                ...
                killOngoingTransOnTable $DB sceprequest
                createIndex $DB idx_scep_sceprequest sceprequest message_time,issuer_dn,subject_dn
                resumeAcceptingNewDBConnections $DB $conn_limit
                applySchemaUsingRole $DB $DB_GROUP $DDLS_PATH/insert.dbversion_2.1.0.ddl
                checkExitCode "Added Index for the table sceprequest in pkirascepdb in version 2.1.0"
                ;&
        "2.1.0") echo "Upgraded to version 2.2.0"
                applySchemaUsingRole $DB $DB_GROUP $DDLS_PATH/insert.dbversion_2.2.0.ddl
                checkExitCode "Role based password authentication when making a connection in pkirascepdb in version 2.2.0"
                ;&
        "2.2.0") echo "Upgraded to version 2.3.0"
                applySchemaUsingRole $DB $DB_GROUP $DDLS_PATH/insert.dbversion_2.3.0.ddl
                checkExitCode "Role based password authentication in pkirascepdb in version 2.3.0"
                ;&
    esac
}
