...
# $2 refers to ddl file
#*****************************************************************************#
function applySchemaUsingPostgresUser() {
    database=$1
    DDL_FILE=$2
    ...
    if [ -f ${DDL_FILE} ]; then
        PGPASSWORD=$(cat ${FILE_DIR}/${FILE}) ${PG_ROOT}/psql -U${PG_USER} -h${HOSTNAME} -d${database} -q -w -v ON_ERROR_STOP=ON -f ${DDL_FILE} 2>>${INSTALL_LOG_DIR}/${LOG_FILE}
        checkExitCode "applying Schema ${DDL_FILE} to ${database} "
    else
        infoLog " ${DDL_FILE} is missing!"
        exit 1
...
#*****************************************************************************#
# This method checks for existence of a particular extension in the given database
# $1 refers to database name
# $2 refers to extension name
#*****************************************************************************#
function isDBExtensionExists(){
    database=$1
    extension=$2

    DB_EXTENSION_TEST=${TMP_DIR}/${extension}_extension_test

    infoLog "Checking for ${extension} extension in ${database}"
    if [ -f ${DB_EXTENSION_TEST} ]; then
        rm -rf ${DB_EXTENSION_TEST}
    fi

    PGPASSWORD=$(cat ${FILE_DIR}/${FILE}) ${PG_ROOT}/psql -U${PG_USER} -h${HOSTNAME} -d${database} <<EOF 1>${DB_EXTENSION_TEST} 2>/dev/null
    SELECT * FROM pg_available_extensions WHERE name = '${extension}';
EOF
    checkExitCode "Verifying whether ${extension} exists in ${database} "
    is_present=`grep -i ${extension} ${DB_EXTENSION_TEST} | wc -l`
    if [[ ${is_present} -eq 0 ]]; then
        infoLog "pkimanagerdb requires pgcrypto extension, but it is not present in the postgres installation"
        exit 1
    fi
}
