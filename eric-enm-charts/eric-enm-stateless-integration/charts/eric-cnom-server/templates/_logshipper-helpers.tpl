...
*/}}
{{- define "eric-cnom-server.logshipper-global" -}}
...
*/}}
{{- define "eric-cnom-server.logshipper-internal" -}}
...
*/}}
{{- define "eric-cnom-server.logshipper-service-fullname" -}}
...
*/}}
{{- define "eric-cnom-server.logshipper-name" -}}
...
*/}}
{{- define "eric-cnom-server.logshipper-labels" }}
...
*/}}
{{- define "eric-cnom-server.logshipper-image" }}
{{- $productInfo := fromYaml (.Files.Get "eric-product-info.yaml") }}
...
*/}}
{{- define "eric-cnom-server.logshipper-container" -}}
{{- $g := fromJson (include "eric-cnom-server.logshipper-global" .) }}
{{- $default := fromJson (include "eric-cnom-server.logshipper-default-value" .) }}
- name: "logshipper"
  imagePullPolicy: {{ or $default.imageCredentials.logshipper.registry.imagePullPolicy $g.registry.imagePullPolicy }}
  image: {{ include "eric-cnom-server.logshipper-image" . }}
  args:
  ...
  - name: SERVICE_ID
    value: {{ include "eric-cnom-server.logshipper-service-fullname" . }}
  - name: CONTAINER_NAME
    value: logshipper
  livenessProbe:
    exec:
  ...
  - name: "{{ include "eric-cnom-server.logshipper-service-fullname" . }}-logshipper-cfg"
...
*/}}
{{- define "eric-cnom-server.logshipper-storage-path" }}
{{- $default := fromJson (include "eric-cnom-server.logshipper-default-value" .) }}
- name: "eric-log-shipper-storage-path"
  mountPath: {{ $default.logshipper.storagePath | quote }}
{{- end -}}
...
*/}}
{{- define "eric-cnom-server.logshipper-volume" -}}
{{- $g := fromJson (include "eric-cnom-server.logshipper-global" .) }}
{{- $default := fromJson (include "eric-cnom-server.logshipper-default-value" .) }}
- name: "eric-log-shipper-storage-path"
  emptyDir:
...
- name: "lt-client-cert"
  secret:
    secretName: "{{ include "eric-cnom-server.logshipper-service-fullname" . }}-lt-client-cert"
    optional: true
{{- end }}
{{- end -}}
...
*/}}
{{- define "eric-cnom-server.logshipper-tls-cert-lt-client" -}}
{{- $default := fromJson (include "eric-cnom-server.logshipper-default-value" .) -}}
{{- $g := fromJson (include "eric-cnom-server.logshipper-global" .) -}}
{{- if $g.security.tls.enabled -}}
...
metadata:
  name: "{{ include "eric-cnom-server.logshipper-service-fullname" . }}-lt-client-cert"
  labels:
    {{- include "eric-cnom-server.logshipper-labels" . | indent 4 }}
  annotations:
    {{- include "eric-cnom-server.product-info" . | nindent 4 }}
spec:
  kubernetes:
    generatedSecretName: "{{ include "eric-cnom-server.logshipper-service-fullname" . }}-lt-client-cert"
    certificateName: "clicert.pem"
    privateKeyName: "cliprivkey.pem"
  certificate:
    subject:
      cn: {{ include "eric-cnom-server.logshipper-service-fullname" . | quote }}
    issuer:
...
{{- define "eric-cnom-server.logshipper-default-value" -}}
  {{- $productInfo := fromYaml (.Files.Get "eric-product-info.yaml") -}}
  ...
  {{/*CUSTOM: Setting logData here, because it does not make sense to have it configurable*/}}
  {{- $default := merge $default (dict "logshipper" (dict "harvester" (dict "logData" (list (dict "subPaths" (list "eric-cnom-server*")))))) -}}
  {{/*END CUSTOM*/}}
