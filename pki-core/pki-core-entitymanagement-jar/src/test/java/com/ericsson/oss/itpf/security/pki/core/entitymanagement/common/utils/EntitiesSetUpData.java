 ...
 *******************************************************************************
 *----------------------------------------------------------------------------*/
package com.ericsson.oss.itpf.security.pki.core.entitymanagement.common.utils;
...
public class EntitiesSetUpData {
    ...
    private Subject createSubject() {
        ...
        final SubjectField dc = new SubjectField();
        dc.setType(SubjectFieldType.DC);
        dc.setValue("TEL");

        final SubjectField initials = new SubjectField();
        initials.setType(SubjectFieldType.INITIALS);
        initials.setValue("T");

        final SubjectField generation = new SubjectField();
        generation.setType(SubjectFieldType.GENERATION);
        generation.setValue("G2");

        final SubjectField email = new SubjectField();
        email.setType(SubjectFieldType.EMAIL_ADDRESS);
        email.setValue("erbsNode@mail.com");

        subjectFields.add(common_name);
        subjectFields.add(organization);
        subjectFields.add(organizationUnit);
        subjectFields.add(dc);
        subjectFields.add(initials);
        subjectFields.add(generation);
        subjectFields.add(email);
    ...
    /**
     * Creates subject using subject fields map.
     * 
     * @param subMap
     * 
     * @return subject
     */
    public Subject createSubject(HashMap<SubjectFieldType, String> subMap) {

        final Subject subject = new Subject();

        final List<SubjectField> subjectFields = new ArrayList<SubjectField>();
        for (Entry<SubjectFieldType, String> entry : subMap.entrySet()) {
            SubjectField common_name = new SubjectField();
            common_name.setType(entry.getKey());
            common_name.setValue(entry.getValue());
            subjectFields.add(common_name);
        }
        subject.setSubjectFields(subjectFields);
        return subject;
