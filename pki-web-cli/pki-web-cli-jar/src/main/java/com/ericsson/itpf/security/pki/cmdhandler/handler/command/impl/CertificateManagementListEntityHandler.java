...
/**
 * Handler implementation for CertificateManagementListEntity. This provides service to list certificate(s) for end entity based on status
 *
 * "pkiadm" ( "certmgmt" | "ctm" ) "EECert" LIST ENTITY_NAME STATUS LIST ::= ( "--list" | "-l" ) ENTITY_NAME ::= ("--entityname" | "-en") " " <entity_name> STATUS ::= ("--status"|"-s") " " CERT_STATUS
 *
 * @author xpranma
 *
 */

@CommandType(PkiCommandType.ENTITYCERTMANAGEMENTLIST)
@Local(CommandHandlerInterface.class)
public class CertificateManagementListEntityHandler implements CommandHandler<PkiPropertyCommand>, CommandHandlerInterface {
    @Inject
    Logger logger;
    ...
    /**
     * Method implementation of CertificateManagementListEntityHandler. Handles command to list certificate(s) for EndEntity
     *
     * @param command
     *            - the command
    ...
    public PkiCommandResponse process(final PkiPropertyCommand command) {
        ...
        logger.info("ENTITYCERTMANAGEMENTLIST command handler");
        ...
        try {
            ...
            certificates = getEECertificates(entityName.trim(), certificateStatus);
            ...
            logger.debug(PkiErrorCodes.CERTIFICATE_STATUS_NOT_SUPPORTED, invalidEntityAttributeException);
            ...
            logger.error(PkiErrorCodes.UNEXPECTED_INTERNAL_ERROR + Constants.SPACE_STRING + exception.getMessage());
            return prepareErrorMessage(ErrorType.UNEXPECTED_ERROR.toInt(), PkiErrorCodes.UNEXPECTED_INTERNAL_ERROR, exception);
        }
        systemRecorder.recordSecurityEvent("PKIWebCLI.ENTITYCERTMANAGEMENTLIST", "CertificateManagementListEntityHandler",
                "Certificate(s) listed successfully for End Entity : " + entityName.trim(), "List certificate(s) for end entity",
    ...
    private List<Certificate> getEECertificates(final String entityName, final CertificateStatus certificateStatus) throws CertificateNotFoundException, CertificateServiceException,
            EntityNotFoundException, InvalidEntityAttributeException {

        List<Certificate> certificates = null;

        if (certificateStatus != null) {
            certificates = eServiceRefProxy.getEndEntityCertificateManagementService().listCertificates_v1(entityName, certificateStatus);
        }
    ...
    private PkiCommandResponse prepareErrorMessage(final int errorCode, final String errorMessage, final Throwable cause) {
        ...
        return new PkiMessageCommandResponse(CliUtil.buildMessage(errorCode, errorMessage), PkiWebCliException.ERROR_CODE_START_INT + errorCode, errorMessage,
