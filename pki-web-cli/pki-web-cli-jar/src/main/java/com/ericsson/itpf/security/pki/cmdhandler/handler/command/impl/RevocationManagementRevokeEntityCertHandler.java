...
/**
 *
 * Handler implementation for RevocationManagementRevokeEntityCert. This provides service to revoke certificate for End Entity
 *
 * "pkiadm" ( "revmgmt" | "rem" ) "EE" REVOKE ENTITY_IDENTIFICATION REVOCATION_REASON REVOKE INVALIDITY_DATE ::= ( "--revoke" | "-rev" ) ENTITY_IDENTIFICATION ::= NAME | CERTIFICATE IDENTIFIER
 ...
 * "aACompromise" ) INVALIDITY_DATE ::=("--invalidityDate" |"--invalDate") " "<YYYY-MM-DD HH:mm:ss > Note:[ ] indicate optional parameter (non manadatory)
 * @author xsaufar
 *
 */

@CommandType(PkiCommandType.REVOCATIONMANAGEMENTREVOKEENTITYCERT)
@Local(CommandHandlerInterface.class)
public class RevocationManagementRevokeEntityCertHandler implements CommandHandler<PkiPropertyCommand>, CommandHandlerInterface {
    ...
    /**
     * Method implementation for RevocationManagementRevokeEntityCertHandler.Processes the command to revoke the certificate from service.
     *
     * @param command
     *
    ...
    public PkiCommandResponse process(final PkiPropertyCommand command) {
        logger.info("REVOCATIONMANAGEMENTREVOKEENTITYCERT command handler");
        ...
        try {
            ...
            if (command.hasProperty(Constants.ENTITYNAME)) {
                ...
                if (revocationReason != null) {
                    systemRecorder.recordSecurityEvent("PKIWebCLI.REVOCATIONMANAGEMENTREVOKEENTITYCERT",
                            "RevocationManagementRevokeEntityCertHandler", "Certificate revoked of entity: " + entityName + " due to reason: "
                                    + revocationReason.toString(), "Revoke entity certificate", ErrorSeverity.INFORMATIONAL, "SUCCESS");
                }
            }
                ...
                systemRecorder.recordSecurityEvent("PKIWebCLI.REVOCATIONMANAGEMENTREVOKEENTITYCERT", "RevocationManagementRevokeEntityCertHandler",
                        "Certificate revoked of issuer: " + issuerName + " and serial number: " + serialNumber, "Revoke entity certificate",
                ...
                systemRecorder.recordSecurityEvent("PKIWebCLI.REVOCATIONMANAGEMENTREVOKEENTITYCERT", "RevocationManagementRevokeEntityCertHandler",
                        "Certificate revoked successfully for subjectDN " + subjectDN + " ,issuerDN: " + issuerDN + " and serial number: " + serialNo,
                        "Revoke entity certificate", ErrorSeverity.INFORMATIONAL, "SUCCESS");
            }
            ...
            return prepareErrorMessage(ErrorType.INVALID_ENTITY_ATTRIBUTE_EXCEPTION.toInt(), PkiErrorCodes.INVALID_ENTITY);
        ...
        } catch (IssuerCertificateRevokedException issuerCertificateRevokedException) {
            ...
            logger.debug(PkiErrorCodes.INVALID_DATE_FORMAT, invalidInvalidityDateException);
    ...
    private String revokeCertificate(final String entityName, final RevocationReason revocationReason, final Date invalidityDate)
 throws CertificateNotFoundException, EntityNotFoundException,
            ExpiredCertificateException, InvalidInvalidityDateException, RevokedCertificateException, RevocationServiceException, RootCertificateRevocationException {
        String returnMsg = Constants.EMPTY_STRING;

        eServiceRefProxy.getRevocationService().revokeEntityCertificates(entityName, revocationReason, invalidityDate);
    ...
    private String revokeCertificate(final String subjectDN, final String issuerDN, final String serialNo, final RevocationReason revocationReason, final Date invalidityDate)
        ...
        dNBasedCertificateIdentifier.setCerficateSerialNumber(serialNo);

        eServiceRefProxy.getRevocationService().revokeCertificateByDN(dNBasedCertificateIdentifier, revocationReason, invalidityDate);

        returnMsg += "Certificate with Subject DN:: " + subjectDN + Constants.REVOKED_SUCCESSFULLY;
