...
/**
 * <p>
 * Handler implementation for TrustManagementListByStatusHandler. This provides service to List certificate(s) for CA/EE entity based on status.
 * </p>
 *
 * "pkiadm" ( "trustmgmt" | "tsm" ) LIST ENTITY_TYPE [CERT_STATUS]
 *
 * LIST ::= ( "--list" | "-l" )
 *
 ...
 * ENT_TYPE ::= "CA" | "EE"
 *
 * CERT_STATUS ::= ( "--certstatus"|"-cs" ) CERT_STATUS_VALUE
 *
 * CERT_STATUS_VALUE ::= ( "active" | "inactive" )
 *
 * @author tcsviga
 *
 */
@CommandType(PkiCommandType.TRUSTMANAGEMENTLISTBYSTATUS)
@Local(CommandHandlerInterface.class)
public class TrustManagementListByStatusHandler implements CommandHandler<PkiPropertyCommand>, CommandHandlerInterface {
    ...
    /**
     *  Method implementation of TrustManagementListByStatusHandler. Processes the command to perform list published certificates operation based on status for CA/END Entity.
     *
     * @param command
     *
    ...
    public PkiCommandResponse process(final PkiPropertyCommand command) {
        logger.debug("TRUSTMANAGEMENTLISTBYSTATUS command handler");
        ...
        try {
            ...
            trustedEntityInfoList = eServiceRefProxy.getEntityManagementService().getTrustedEntitiesInfo(entityType, certificateStatus);
            systemRecorder.recordSecurityEvent("PKIWebCLI.TRUSTMANAGEMENTLISTBYSTATUS", "TrustManagementListByStatusHandler",
                    "Fetched list of published certificates of entity type: " + entityType.getValue() + " and status: " + certificateStatus.name(),
                    "List published certificates based on type and status", ErrorSeverity.INFORMATIONAL, "SUCCESS");

        } catch (final EntityServiceException entityServiceException) {
            return prepareErrorMessage(ErrorType.ENTITY_SERVICE_EXCEPTION.toInt(), PkiErrorCodes.UNABLE_LIST_CERTIFICATE + Constants.SPACE_STRING + PkiErrorCodes.SERVICE_ERROR
                    + PkiErrorCodes.SUGGEST_SOLUTION + PkiErrorCodes.RETRY, entityServiceException);
        } catch (final EntityNotFoundException entityNotFoundException) {
            return prepareErrorMessage(ErrorType.ENTITY_NOT_FOUND_EXCEPTION.toInt(), PkiErrorCodes.ENTITY_DOES_NOT_EXIST + Constants.SPACE_STRING + entityNotFoundException.getMessage());
        } catch (final CertificateNotFoundException certificateNotFoundException) {
            return prepareErrorMessage(ErrorType.CERTIFICATE_NOT_FOUND.toInt(), PkiErrorCodes.NO_CERTIFICATE_FOUND + Constants.SPACE_STRING + certificateNotFoundException.getMessage());
        } catch (final InvalidEntityException invalidEntityException) {
            return prepareErrorMessage(ErrorType.INVALID_ENTITY_EXCEPTION.toInt(), PkiErrorCodes.NO_CERTIFICATE_FOUND + Constants.SPACE_STRING + invalidEntityException.getMessage());
        } catch (TrustDistributionPointURLNotFoundException trustDistributionPointURLNotFoundException) {
    ...
    private PkiCommandResponse buildCommandResponse(final List<TrustedEntityInfo> trustedEntityInfoList) {
        if (ValidationUtils.isNullOrEmpty(trustedEntityInfoList)) {
            return prepareErrorMessage(ErrorType.CERTIFICATE_NOT_FOUND.toInt(), PkiErrorCodes.CERTIFICATE_WITH_STATUS_NOT_FOUND);
        }
    ...
    private String[] getTrustedEntityInfoDetails(final TrustedEntityInfo trustedEntityInfo) {
        ...
        final String[] certificateDetails = { trustedEntityInfo.getEntityType().toString(), Constants.EMPTY_STRING + trustedEntityInfo.getCertificateSerialNumber(),
    ...
    private CertificateStatus getCertificateStatus(final PkiPropertyCommand command) throws IllegalArgumentException {
        if (!command.hasProperty(Constants.CERT_STATUS)) {
            throw new IllegalArgumentException(PkiErrorCodes.UNSUPPORTED_CERTIFICATE_STATUS);
        }

        final String status = command.getValueString(Constants.CERT_STATUS);
        CertificateStatus certificateStatus = null;

        try {
            certificateStatus = commandHandlerUtils.getCertificateStatus(status);

            if (certificateStatus.equals(CertificateStatus.ACTIVE) || certificateStatus.equals(CertificateStatus.INACTIVE)) {
                return certificateStatus;
            }
        } catch (final IllegalArgumentException illegalArgumentException) {
            throw new IllegalArgumentException(PkiErrorCodes.UNSUPPORTED_CERTIFICATE_STATUS, illegalArgumentException);
        }
        return certificateStatus;
    }
