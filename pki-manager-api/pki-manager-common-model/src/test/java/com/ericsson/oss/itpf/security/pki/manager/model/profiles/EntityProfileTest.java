...
import com.ericsson.oss.itpf.security.pki.manager.test.setup.EntityProfileSetUpData;

/**
 * This class is used to run Junits for EntityProfile objects in different scenarios
 */
public class EntityProfileTest extends EqualsTestCase {

    private static final String SET_CERTIFICATE_PROFILE = "setCertificateProfile";
    private static final String SET_PROFILE_VALIDITY = "setProfileValidity";
    ...
    protected Object createInstance() throws DatatypeConfigurationException {
        return new EntityProfileSetUpData().getEntityProfileForEqual();
    }
    ...
    protected Object createNotEqualInstance() throws DatatypeConfigurationException {
        return new EntityProfileSetUpData().getEntityProfileForNotEqual();
    }
        ...
        for (final Method method : methods) {
            if (method.getName().startsWith(SET_STRING)) {
                if (method.getParameterTypes()[0].getName().contains(".") && !method.getParameterTypes()[0].isEnum()) {
                    if (!method.getName().equals(SET_CERTIFICATE_PROFILE) && !method.getName().equals(SET_PROFILE_VALIDITY)) {
    ...
    @Override
    @Test
    public void testWithEmptyList() throws IllegalAccessException, IllegalArgumentException, InvocationTargetException, DatatypeConfigurationException {
        final Object eq1 = createInstance();
        final Class tClass = eq1.getClass();
        final Method[] methods = tClass.getMethods();
        Object tempObject1 = createInstance();
        ...
        for (final Method method : methods) {
            if (method.getName().startsWith(SET_STRING)) {
                if (method.getParameterTypes()[0].getName().contains(LIST_STRING) && !method.getParameterTypes()[0].isEnum() && !method.getName().startsWith("setTrustProfiles")) {
