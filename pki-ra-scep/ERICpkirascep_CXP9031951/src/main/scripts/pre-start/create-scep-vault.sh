...
VAULT_INFO_FILE=/ericsson/security/data/vault_info_scep.txt                     #File to store content of vault that need to added in standalone
...
PWD_TO_BE_STORED=C4bCzXyT              #Actual password to be stored
ATTRIBUTE_NAME=SecuredAttributeName    #Attribute name stored s reference
BLOCK_NAME=SCEP_VAULT                  #Block name of the password stored
...
if grep -Fq "SCEP_VAULT" $STANDALONE_PATH
then
    echo "SCEP vault related system properties already exits in standalone-enm.xml"
else
	create_keystore
	save_passwords "RA_KEYSTORE_SCEP"
	update_standalone
fi
