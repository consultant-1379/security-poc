...
public class KeyUpdateResponseHandlerTest {
    ...
    public static void initializeTestData() throws IOException {
        ...
        final PKIMessage expectedPKIRequestMessage =
                RequestGeneratorFactory.getRequestGenerator(RequestType.KEY_UPDATE_REQUEST).generate(requestParameters, null);
        ...
        final PKIMessage expectedPKIResponseMessage =
                ResponseGeneratorFactory.getResponseGenerator(
                        com.ericsson.oss.itpf.security.pki.common.test.response.generator.ResponseType.KEY_UPDATE_RESPONSE)
                        .generate(expectedPKIRequestMessage, responseParameters);

        pKIRequestMessage = new RequestMessage(expectedPKIRequestMessage.getEncoded());
        pKIResponseMessage = new KeyUpdateResponseMessage(expectedPKIResponseMessage.getEncoded());
    ...
    public void testHandle() throws IOException {
        ...
        keyUpdateResponseHandler.handle(cMPResponse);
    ...
    private void setUpTestData() throws IOException {
        final byte[] keyUpdateResponseFromManager = pKIResponseMessage.toByteArray();
        Mockito.when(cMPResponse.getTransactionID()).thenReturn(transactionId);
        Mockito.when(cMPResponse.getCmpResponse()).thenReturn(keyUpdateResponseFromManager);
    ...
    public void testHandleSyncResponse() throws ResponseSignerException, IOException {
        ...
        Mockito.when(messageSignerService.signMessage(Matchers.anyString(), Matchers.<KeyUpdateResponseMessage>anyObject()))
